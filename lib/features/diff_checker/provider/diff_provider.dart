...
final diffOptionsProvider = StateProvider<DiffOptions>((ref) => const DiffOptions());
final diffResultProvider = StateProvider<List<dmp.Diff>>((ref) => []);
final diffSummaryProvider = StateProvider<DiffSummary?>((ref) => null);

final diffSourceLeftProvider = StateProvider<DiffSource>((ref) => DiffSource.text);
final diffSourceRightProvider = StateProvider<DiffSource>((ref) => DiffSource.text);
...
void computeDiff(WidgetRef ref) {
  ...
  final diffs = DiffUtils.computeDiff(formattedLeft, formattedRight, options: options);
  ref.read(diffResultProvider.notifier).state = diffs;
  ref.read(diffSummaryProvider.notifier).state = DiffUtils.calculateSummary(diffs);
}
